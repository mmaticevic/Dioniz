...
if(isset($_GET["id"])){
	...
	$izraz=$veza->prepare("select count(id) from vinarija where mjesto=:id");
	$izraz->execute($_GET);
	$ukupno = $izraz->fetchColumn();

	$izraz=$veza->prepare("select count(id) from osoba where mjesto=:id");
	$izraz->execute($_GET);
	$ukupnoo = $izraz->fetchColumn();	
}
...
if(isset($_POST["obrisi"])){	
	...
	unset($_POST["obrisi"]);
	$izraz=$veza->prepare("delete from mjesto 
		where id=:id");
	$izraz->execute($_POST);
		//print_r($_POST);
		//exit;
					...
					<div class="features_items"><!--features_items-->
						...
						<div class="signup-form">
							<form method="POST" action="<?php echo $_SERVER["PHP_SELF"] ?>">
								...
								<?php if($ukupno==0 && $ukupnoo==0): ?>

									<button type="submit" name="obrisi" class="btn btn-default" value="Brisanje">Obriši</button>
								<?php else:?>
									<div class="alert alert-warning">
    <strong>Upozorenje!</strong>  Mjesto se ne može obrisati jer je na njoj prijavljena vinarija ili osoba!
  </div>
								<?php endif;?>
