...
if(isset($_GET["id"])){
	...
	$izraz=$veza->prepare("SELECT count(id) from vino where vinarija=:id");
	$izraz->execute($_GET);
	$ukupnovina = $izraz->fetchColumn();
}
...
if(isset($_POST["obrisi"])){
		...
		$izraz=$veza->prepare("DELETE from vinarija where id=:id");
		$izraz->execute($_POST);
...
<section>
    ...
    <strong>Upozorenje!</strong> <br> Vinarija se ne može obrisati!! <br><hr>Da bi obrisali vinariju morate obrisati sva vina!!!
  </div>
					<?php endif;?>
